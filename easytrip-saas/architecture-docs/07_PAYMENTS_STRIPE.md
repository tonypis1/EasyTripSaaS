...
1. **Stripe Dashboard** → *Catalogo prodotti* → *+ Aggiungi prodotto*.
2. Compilare:
...
- Stripe Dashboard → *Developers* → *Webhooks* → *Send test webhook* `checkout.session.completed`: deve restituire 2xx e creare un `Payment` (idempotente per `stripePaymentId`).

## 8. Promotion Code: sconto 20% "Nuovo viaggio"

### 8.1 Idea

L'utente che ha **appena completato un viaggio** (entro 7 giorni dalla data di fine) e torna sull'app per crearne un altro, vede uno sconto del **20% direttamente in checkout Stripe**. È una leva di retention basata sulla *peak-end rule*: il piacere del viaggio appena finito è ancora vivo, e uno sconto immediato spinge alla riprogettazione.
...
1. **Stripe Dashboard** → *Catalogo prodotti* → *Coupons* → *+ Nuovo coupon*.
   - **Nome interno**: `easytrip-new-trip-20`
   - **Tipo**: `Percentuale di sconto`
   - **Sconto**: `20%`
   - **Durata**: `Una volta` (one-off)
2. Salvare → poi *Promotion codes* → *+ Nuovo codice promozionale* legato al coupon.
...
- In alternativa: disattivare il Promotion Code da Stripe Dashboard (più rapido se serve un kill-switch *senza* redeploy). Il backend continuerà a passarlo a Stripe ma Stripe risponderà con un errore di validazione: per evitare 500 in checkout, **preferire** la disattivazione tramite env.
...
1. Click su CTA "Iniziamo subito" del piano *Viaggiatore Frequente*.
...
5. Se la creazione fallisce (env mancante, Stripe down, abbonamento già attivo) → la pagina mostra un fallback con CTA *Riprova* / *Torna alla home*.
