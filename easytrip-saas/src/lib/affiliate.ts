...
export function getYourGuideUrl(query: string, destination: string): string | null {
...
export function theForkUrl(restaurantName: string, city: string): string | null {
...
export function hasAnyAffiliate(): boolean {
  return Boolean(BOOKING_AID || GYG_PARTNER_ID || THEFORK_PARTNER_ID || VIATOR_PID);
}
...
export type AffiliatePartner = "booking" | "gyg" | "thefork" | "viator" | "amazon";

export const PARTNER_LABELS: Record<AffiliatePartner, { name: string; color: string }> = {
  booking: { name: "Booking.com", color: "text-blue-400 border-blue-400/30 bg-blue-500/10" },
  gyg: { name: "GetYourGuide", color: "text-orange-400 border-orange-400/30 bg-orange-500/10" },
  thefork: { name: "TheFork", color: "text-green-400 border-green-400/30 bg-green-500/10" },
  viator: { name: "Viator", color: "text-teal-400 border-teal-400/30 bg-teal-500/10" },
  amazon: { name: "Amazon", color: "text-amber-400 border-amber-400/30 bg-amber-500/10" },
};
